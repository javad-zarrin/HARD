...
static const int Lookup2=7010; // Broadcast lookup
static const int DUpdate2=7011; // Broadcast Reply
static const int Release_Resources2=7012; // Broadcast Release
//-----------------------------------------------------------------------------------------------
...
static const double MAX_TTL=20000;

static const unsigned int Max_Querying_Iterations=10;//4;
//static const unsigned int Desired_Number_Of_Resources=1; //number of target resources of the same type to be discovered 1-6

static const unsigned int number_of_requesters_factor=10; // A=percentage of requesters,  B=100/number_of_requesters_factor, A=B/10
...
long uid(long mq_id, unsigned int sq_id){
    long result=mq_id*10+ (long) sq_id;
    return result;
}
...
//  simsignal_t finishSD;
 ...
 //   pair<std::string, int> reservation_status; //pair(reservation status, thread id)
    ...
    double overal_transcated_data_overlay3;
    ...
    std::map<long, unsigned int> broadcast_states;  //uid for each broadcasted query and number of query replies
    std::map<long, subQuery> broadcast_subQuery_states; //uid for each broadcasted query and number of messages before broadcasting
    std::map<long, rlist> broadcast_discovery_options;
...
protected:
    ...
    int next_qms_selection_v2(subQuery sub_query);
