...
void rdNode::initialize() {
    ...
    overal_transcated_data_overlay3=0.0;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        case SuperNodeElection: {
            ...
            if (ttmsg->getDestination() == vnode_id) {
                if (ttmsg->getSource()!=vnode_id) {num_rec_overlay3_msg++; overal_transcated_data_overlay3+=2*sizeof(strtype)+7*sizeof(int)+sizeof(unsigned int)+sizeof(bool);
                EV<< "overal_transcated_data_overlay3="<< overal_transcated_data_overlay3 << endl;
...
//---------------------------------------------------------------------------------------------------------
        case LS_Request: {
            if (ttmsg->getDestination() == vnode_id) {
                if (ttmsg->getSource()!=vnode_id) {

                    //num_rec_overlay_msg++;overal_transcated_data_overlay+=2*sizeof(strtype)+7*sizeof(int)+sizeof(unsigned int)+sizeof(bool);
                //EV<< "overal_transcated_data_overlay="<< overal_transcated_data_overlay << endl;
...
//---------------------------------------------------------------------------------------------------------
        case SG_Members: {
            if (ttmsg->getDestination() == vnode_id) {
                ...
                resource_key.sstamp=ttmsg->getStringContent().description;
        ...
        case EndingSD: {
            ...
            if (module_role == SN) {
                ...
                for(unsigned int i=0;i<sgroup_members.size();i++){
                    ...
                    st2.description=resource_key.sstamp;
                    smsmsg->setMainContent(st);
                    smsmsg->setStringContent(st2);
                    EV << " j-debug 81 sstamp="<< st2.description << " in vnode_id="<< vnode_id;
...
//---------------------------------------------------------------------------------------------------------
        ...
        case Main_Query: {
            if (ddmsg->getDestination() == (unsigned int) vnode_id) {
               ...
               EV << endl<<"JX: a main query (mqid="<< ddmsg->getMq().main_query_id << ") from " << ddmsg->getSource() << " has arrived in "<< vnode_id<< endl;
               ...
               EV<< endl<< "JX: Main_Query mqid="<<  temp_mq.main_query_id << " list of thread_ids=";




               for(unsigned int i=0; i< ddmsg->getMq().nhogs; i++ ){
                   temp_sq.thread_group_id=temp_mq.main_query_id+ddmsg->getMq().hogroups[i].homog_id;

                   EV << temp_sq.thread_group_id << ",";
               ...
               EV << endl<< "Starting Query Analyzer ..."<<endl;
               for(unsigned int i=0; i< ddmsg->getMq().nhogs; i++ ){
               ...
               //    if ((int) ddmsg->getSource()==vnode_id) sub_query.nMS=0; else sub_query.nMS=2;
                   ...
                   sub_query.thread_group_id=temp_mq.main_query_id+ ddmsg->getMq().hogroups[i].homog_id;
                   ...
                   if (sub_query.qss=="n"){
                       ...
                       long quid=uid(sub_query.main_query_id,sub_query.sub_query_id);
                       broadcast_states[quid]=(unsigned int) agroup_members.size();
                       sub_query.nMS+=(unsigned int) 2*agroup_members.size()-2;
                       broadcast_subQuery_states[quid]=sub_query;
                       if (broadcast_discovery_options.find(quid)!=broadcast_discovery_options.end()) broadcast_discovery_options.erase(quid);

                       EV<<endl<<  "JX: vnode_id="<< vnode_id << " mq_id="<< sub_query.main_query_id <<", sq_id="<< sub_query.sub_query_id << " nMS="<< sub_query.nMS << " am.size="<< agroup_members.size()<< endl;

                       subQuery temp_sub_query;

                       for (unsigned int i = 0; i < agroup_members.size(); i++) {
                           EV << " mem " << i << "=" << agroup_members[i] << endl;
                           temp_sub_query=sub_query;
                           temp_sub_query.DRs.clear();

                           SQM *lmsg;
                           lmsg = new SQM("Lookup2");
                           lmsg->setDestination(agroup_members[i]);
                           lmsg->setSource(vnode_id);
                           lmsg->setMtype(Lookup2);

                           lmsg->setSq(temp_sub_query);

                           sendMessage(lmsg);
                           EV << endl << "JX: in vnode_id="<<vnode_id<< "  a sub_query (broadcast lookup request with "<< "mq_id="<< sub_query.main_query_id <<", sq_id="<< sub_query.sub_query_id << ")"<<" from " << vnode_id << " has been sent to "<< agroup_members[i]<< endl;
                       ...
                       if (key_checking(sub_query.qss,resource_key.sstamp)) {
                           ...
                           for (unsigned int i = 0; i < agroup_members.size(); i++) {
                               ...
                               int next_qms=next_qms_selection_v2(sub_query);
...
//---------------------------------------------------------------------------------------------------------
        ...
        case Lookup2: {
            if (sqmsg->getDestination() == (unsigned int) vnode_id) {
                ...
                EV << endl << "JX: in vnode_id="<<vnode_id<< "  a sub_query (broadcast lookup request with "<< "mq_id="<< temp_query_request.main_query_id <<", sq_id="<< temp_query_request.sub_query_id << ")"<<" from " << sqmsg->getSource() << " has been arrived in "<< vnode_id<< endl;
...
//===================================================
                ...
                unsigned number_of_available_resources;

                if (resource_key.lstamp==sqmsg->getSq().qls){
//------------------------------------------------------------------------
                    number_of_available_resources=0;
                       ...
                       number_of_matched_resources=0;
                        for (unsigned i=0; i<resources.size(); i++) {
                            if ((resources[i].thread_group_id==-1) && (temp_query_request.nRR>number_of_matched_resources)) {
                                ...
                                EV << endl << "JX: in vnode_id="<<vnode_id<< "  resource_id=" << resources[i].resource_id << " thread_group_id="<< resources[i].thread_group_id<< endl;
                            }
                        }
                  ...
                  //      reservation_status.first="reserved";
                   //     reservation_status.second=-1;
                       ...
                       // show_sors_status("succ-Lookup", vnode_id, temp_query_request); // optional debug
                        ...
                        EV<< endl<<"JX: in vnode_id="<< vnode_id << " number of reserved resources=" << number_of_matched_resources << " which are: ";
                        for(unsigned int i=0; i< temp_query_request.DRs.size();i++){
                            EV << temp_query_request.DRs[i]<<",";
                        }
                        EV << endl;

                        sendMessage(dumsg);

                        EV << endl << "JX: in vnode_id="<<vnode_id<< "  a sub_query (broadcast d-update msg with "<< "mq_id="<< temp_query_request.main_query_id <<", sq_id="<< temp_query_request.sub_query_id << ")"<<" from " << vnode_id << " has been sent to "<< qms_id<< endl;
...
//---------------------------------------------------------------------------------------------------------
                ...
                case DUpdate2: {
                    if (sqmsg->getDestination() == (unsigned int) vnode_id) {
                        ...
                        subQuery temp_sub_query=sqmsg->getSq();
                        subQuery sub_query;

                        long quid=uid(temp_sub_query.main_query_id, temp_sub_query.sub_query_id);

                        EV << endl << "JX: in vnode_id="<<vnode_id<< "  a sub_query (broadcast d-update msg with "<< "mq_id="<< temp_sub_query.main_query_id <<", sq_id="<< temp_sub_query.sub_query_id << ")"<<" from " << sqmsg->getSource() << " has been arrived in "<< vnode_id<< endl;
                        EV << endl<< "JX: vnode_id="<< vnode_id << " broadcast_states=" << broadcast_states[quid]<< endl;



                        if (broadcast_states.find(quid)==broadcast_states.end()) {
                            EV << " Error 1 in Broadcasting.."<< endl;
                            break;

                        }
                        else {
                            broadcast_states[quid]--;
                        }

                        if (!temp_sub_query.DRs.empty())
                            broadcast_discovery_options[quid].insert( broadcast_discovery_options[quid].end(),temp_sub_query.DRs.begin(), temp_sub_query.DRs.end() );





                        if (broadcast_states[quid]==0) { //QMS update


                            if (broadcast_subQuery_states.find(quid)==broadcast_subQuery_states.end()) {

                                EV << " Error 2 in Broadcasting.."<< endl;
                                break;
                            ...
                            sub_query=broadcast_subQuery_states[quid];

                            if (std::find(sub_query.visited_qms_ids.begin(), sub_query.visited_qms_ids.end(), qms_id )==sub_query.visited_qms_ids.end())
                                sub_query.visited_qms_ids.push_back(qms_id);

                            if (broadcast_discovery_options.find(quid)!= broadcast_discovery_options.end()) {



                                rlist temp_rlist=broadcast_discovery_options[quid];

                                for(unsigned int i=0; i< temp_rlist.size();i++){
                                    if (sub_query.nDR < sub_query.nRR) {
                                        sub_query.DRs.push_back(temp_rlist[i]);
                                        sub_query.nDR++;
                                        EV  << endl<<"JX: vnode_id="<< vnode_id << " resource_id=" << temp_rlist[i]<< " is added to list of discovered resources" << endl;
                                        ...
                                        Signal *smsg;
                                        smsg = new Signal("B-Release");

                                        unsigned int destination_id= (unsigned int) temp_rlist[i]/100;
                                        smsg->setValue(temp_rlist[i]);
                                        smsg->setDestination(destination_id);
                                        smsg->setSource(vnode_id);
                                        smsg->setMtype(Release_Resources2);
                                        sendMessage(smsg);
                                        EV  << endl<<"JX: vnode_id="<< vnode_id << " a b-release_resources signal  for resource_id=" << temp_rlist[i] <<" from " << vnode_id << " has been sent to "<< destination_id<< endl;
                                ...
                                broadcast_discovery_options.erase(quid);


                            }

                            broadcast_states.erase(quid);
                            broadcast_subQuery_states.erase(quid);



                            EV <<endl << "JX: Discovery Reports in vnode_id="<< vnode_id << " for mqid=" << sub_query.main_query_id  << " and sqid="<< sub_query.sub_query_id<< " discovered resources are:";
                            for (unsigned int i=0; i< sub_query.DRs.size(); i++){
                                EV <<sub_query.DRs[i] << ",";
                            }

                            EV << endl <<  " nDR="<< sub_query.nDR << " nRR="<< sub_query.nRR<< endl;

                            double current_time=simTime().dbl();
                            double living_time= (current_time - sqmsg->getSq().TTL)*1000.0;
                            EV << "qid="<< sub_query.main_query_id << ":" << sub_query.sub_query_id << "qid="<< sub_query.main_query_id<<":"<< sub_query.sub_query_id<< " => living time (ms) =" <<  living_time << endl;
                            EV << "qid="<< sub_query.main_query_id << ":" << sub_query.sub_query_id << "CT="<<current_time<< "  and  ST="<< sub_query.TTL << endl;

                            if ((sub_query.nDR < sub_query.nRR) && (living_time < MAX_TTL )){




                                int next_qms=next_qms_selection_v2(sub_query); // if next_qms=-2 : current sqms has fully explored, the sub query must be directed to the upper layer i.e. sqms layer
                                // return=-3: call back to the parent-sender
                                EV << "qid="<< sub_query.main_query_id << ":" << sub_query.sub_query_id << "DUpdate=QMS Update --> next_qms="<< next_qms<< " in qmid="<< sub_query.main_query_id<< ":"<<sub_query.sub_query_id<< endl;
                                if (next_qms==-3){
                                    ...
                                    if (destination_id!=vnode_id) sub_query.nMS++;
                                    lmsg->setSq(sub_query);
                                    ...
                                    sendMessage(lmsg);
                                    EV << "qid="<< sub_query.main_query_id << ":" << sub_query.sub_query_id << "a sub_query (call back downward request with "<< "mq_id="<< sub_query.main_query_id <<", sq_id="<< sub_query.sub_query_id << ")"<<" from " << vnode_id << " has been sent to "<< destination_id<< endl;
                                }
                                else if (next_qms==-2){


                                    int destination_id=QROUT.get_parent_sender_id(sub_query.main_query_id, sub_query.sub_query_id);
                                    if (destination_id==-2) {destination_id=vnode_id;EV <<" j-debug code 14 -->"<< endl;}
                                    sub_query.UPT=6; // no update
...
//-----------------------------------------------------------
                                ...
                                if ((destination_id==-2)&&(QREG.isMQueryRegistered(sub_query.main_query_id)))  {
                          ...
                          //          sub_query_pta_update(sub_query, sqmsg->getSource(), 2);
                                    ...
                                    if ((QREG.Discovery_is_Completed(sub_query.main_query_id)) ||(living_time>= MAX_TTL) || (QREG.MQ_Resolution_State(sub_query.main_query_id) )) {
                                        ...
                                        EV<< endl<< "JX: A main-query is completed in vnode_id="<< vnode_id << "mq_id="<< sub_query.main_query_id <<", sq_id="<< sub_query.sub_query_id << " tg_id="<< sub_query.thread_group_id;
                                        ...
                                        EV << endl<< "JX: A Main Reply with "<< "mq_id="<< final_main_query_status.main_query_id <<" from " << vnode_id << " has been sent to "<< destination_id<< endl;

                                        EV<< endl<< "JX: list of thread_ids=";
                                        for (unsigned int i=0; i< final_main_query_status.sqs.size(); i++){
                                            EV<< final_main_query_status.sqs[i].thread_group_id << ",";
...
//---------------------------------------------------------------------------------------------------------
        ...
        case Release_Resources2 : {
            if (sigmsg->getDestination() == (unsigned int) vnode_id) {

                long int resource_id=(long int) sigmsg->getValue();

                EV << endl << "JX: in vnode_id="<<vnode_id<< "  a resource_release signal for resource_id=" << resource_id<< "from " <<  sigmsg->getSource() << " has been arrived in "<< vnode_id<< endl;


                for (unsigned int i=0; i< resources.size();i++){
                    if (resources[i].resource_id==resource_id) {
                        resources[i].thread_group_id=-1;
                        EV << endl << "JX: in vnode_id="<<vnode_id<< "  resource_id=" << resources[i].resource_id << " has been released."<< endl;
        ...
        case Release_Resources : {
            if (sigmsg->getDestination() == (unsigned int) vnode_id) {
                EV << endl<< "JX: vnode_id="<< vnode_id <<" a resource_release signal from " << sigmsg->getSource() << " has arrived in "<< vnode_id<< endl;
                int tg_id=sigmsg->getValue();
                for (unsigned int i=0; i< resources.size();i++){
                    if (resources[i].thread_group_id==tg_id) {
                        EV << endl << "JX: in vnode_id="<<vnode_id<< "  resource_id=" << resources[i].resource_id << " with thread_group_id="<< resources[i].thread_group_id << " will be released"<< endl;
                        resources[i].thread_group_id=-1;
           ...
           case Ending_Excution : {
               if (lsmsg->getDestination() == (unsigned int) vnode_id) {
                   ...
                   signaled_des.clear();
                   EV << endl<< "JX: an Ending_Execution with mqid="<< mq_reply.main_query_id << " from " << lsmsg->getSource() << " has been arrived in "<< vnode_id<< endl;

                   EV<< endl<< "JX: list of thread_ids=";
                   for (unsigned int i=0; i< mq_reply.sqs.size(); i++){
                       EV<< mq_reply.sqs[i].thread_group_id << ",";
                       ...
                       for (unsigned int j=0; j<lsq.drs.size();j++ ){
                          ...
                          // int destination_id=(int) lsq.drs[j] /10;
                           ...
                           if (std::find(signaled_des.begin(),signaled_des.end(), destination_id)== signaled_des.end()) {
                               ...
                               sendMessage(smsg);
                               EV << "JX: qid="<< mq_reply.main_query_id << " A Final Release_Resources Signal for thread_group_id="<< value<<  " from " << vnode_id << " has been sent to "<< destination_id<< endl;
           ...
           case Main_Reply : {
               if (lsmsg->getDestination() == (unsigned int) vnode_id) {
                   ...
                   EV << endl<< "JX: A Main Reply with "<< "mq_id="<< mq_reply.main_query_id <<" from " << lsmsg->getSource() << " arrived in "<< vnode_id<< endl;
                   ...
                   EV<< endl<< "JX: list of discovered resources=";
                   for (unsigned int i=0; i< mq_reply.sqs.size(); i++){
                       for (unsigned j=0; j<mq_reply.sqs[i].drs.size(); j++)
                           EV<< mq_reply.sqs[i].drs[j] << ",";
...
*/
                 ...
                 // string name1 = "Test-"+ss.str()+ "-Probability-" + hh.str()+".data";
                  string name2 = "Test-"+ss.str()+ "-BRW2" + hh.str()+".txt";



                 /* ofstream bin_result_file (name1.c_str(), ios::app|ios::binary);
                  bin_result_file.write((char*)&temp_res, sizeof(temp_res) );
                  bin_result_file.close();*/

                  ofstream txt_result_file (name2.c_str(),  ios::app|ios::ate);
                   if (txt_result_file.is_open())
                   {
                       txt_result_file << rate_of_requesters<< "," << (unsigned int) par("Desired_Number_Of_Resources")<< "," <<  query_homogenity_rate << "," << qrecord_time << "," << temp_res.number_of_resources << "," << temp_res.time_period << ","<< temp_res.number_of_messages<< ","<<  temp_res.discovery_latency << "," << temp_res.discovery_resolution_rate<< endl ;
...
//    if (sub_query.nDR!=0){
        if (QREG.isMQueryRegistered(sub_query.main_query_id)){
            EV<< endl<<"JX: vnode_id="<< vnode_id << " Input sub_query_registry_update for mqid="<<sub_query.main_query_id << " and sq_id=" << sub_query.sub_query_id << " list of discovered resources=";
           ...
           // if (sub_query.nDR!=0) {
            QREG.registry[sub_query.main_query_id].sqs[sub_query.sub_query_id].drs.insert( QREG.registry[sub_query.main_query_id].sqs[sub_query.sub_query_id].drs.end(),sub_query.DRs.begin(),sub_query.DRs.end());
            //}
        ...
        EV<< "JX:  inside sub_query_registry_update mqid=" << sub_query.main_query_id << " and sq_id="<< sub_query.sub_query_id<<" => from " << QREG.MainQueryResults(sub_query.main_query_id).source << " has been resolved in the following resource-ids by transacting "<< QREG.MainQueryResults(sub_query.main_query_id).sqs[sub_query.sub_query_id].nmsg<<  endl;
        ...
        for(unsigned int i=0; i< QREG.registry[sub_query.main_query_id].sqs[sub_query.sub_query_id].drs.size(); i++){
            EV<< "JX: resource_id="<<QREG.registry[sub_query.main_query_id].sqs[sub_query.sub_query_id].drs[i] << ",";
...
map<unsigned int, double> temp_an_neigbors, temp_an_probabilities;
temp_an_neigbors.insert(an_neigbors.begin(),an_neigbors.end());
temp_an_probabilities.insert(an_neigbors.begin(),an_neigbors.end());


unsigned int total_number_of_qms_nodes_in_current_sqms= total_number_of_qms_in_the_system  ;///scenario1 (unsigned int ) sgroup_members.size();
...
// invoking probability tables in order to determine the next QMS
    ...
    for (std::map<unsigned int,double>::iterator itm=temp_an_neigbors.begin(); itm!=temp_an_neigbors.end(); ++itm) itm->second=0.0;
...
int rdNode::next_qms_selection_v2(subQuery sub_query){
an_neighbors_cleanup();

map<unsigned int, double> temp_an_neigbors;
temp_an_neigbors.insert(an_neigbors.begin(),an_neigbors.end());
...
// invoking probability tables in order to determine the next QMS
    ...
    if (temp_an_neigbors.size()==0) return -3;
        ...
        if (next_qms_cans.size()==0) return -3; else { // return=-3: call back to the parent-sender
            ...
            int randomIndex = rand() % next_qms_cans.size();
            next_qms=next_qms_cans[randomIndex];
            EV << "Random selected neighbor : "<< next_qms << endl;
            return next_qms;
...
void rdNode::an_neighbors_cleanup() {
    ...
    for(std::map<unsigned int,double>::iterator it=an_neigbors.begin(); it!=an_neigbors.end(); ++it){
   //scenario1     if (find(sgroup_members.begin(), sgroup_members.end(), it->first)==sgroup_members.end()){
    ///scenario1        an_neigbors.erase(it);
     ///scenario1   }
    }
}
...
void rdNode::finish() {
    ...
    //   cTopology::Node *nodeA = topo.getNode(vnode_id);

//    ev << "Node A i=" << vnode_id << " is " << nodeA->getModule()->getFullPath() << endl;
    //   cTopology::Node *nodeB = topo.getNodeFor(this);
//    ev << "Node B i=" << vnode_id << " is " << nodeB->getModule()->getFullPath() << endl;

//    EV<< "vnode_id=" << vnode_id  << ", Sent=" << numSent <<  ", Received=" << numReceived << endl;

    //  recordScalar("nsent", numSent);
    //   recordScalar("nreceived", numReceived);
    //   recordScalar("nrouted", numPRouting_Messages);
   ...
   //     recordScalar("nnodes", topo.getNumNodes());
   //     recordScalar("gsize", group_size);
   //     recordScalar("valid", valid);
   //     recordScalar("count", count);

    }

    EV << "vnode_id=" << vnode_id << " and sibling_vnode_id="<< sibling_vnode_id << " List of local occupied resources= ";
    for (unsigned int i=0; i< resources.size();i++){
        if (resources[i].thread_group_id!=-1) EV << resources[i].resource_id<< "("<< resources[i].thread_group_id << ")" << " , ";
    }
    EV<< endl;
    ...
    if (module_role == AN) {
        ...
        EV << "-----------------------------------\n";
        EV << "QMS-Start\n";
        EV<< "total_number_of_qms_in_the_system="<<total_number_of_qms_in_the_system << endl;
        ...
        for (unsigned i = 0; i < agroup_members.size(); i++)
            EV << agroup_members[i] << ", ";
        EV << endl;
        /*     string st=convert_vector_string(agroup_members);
         EV << "String=" << st.c_str() << endl;
         EV << "Vector=" ;
         vector<int> vi=convert_string_vector(st);
         for (unsigned i=0; i<vi.size(); i++) EV<< vi[i] << ", ";
         EV << endl;
         */
...
//emit(finishSD,num_in_msg );
   ...
   string name3 = "Test-"+ss.str()+ "-BRW2-Traffic" + hh.str()+".txt";

   int number_of_resources=55*topo.getNumNodes();

   ofstream txt_result_file (name3.c_str(),  ios::app|ios::ate);
    if (txt_result_file.is_open())
    {
        txt_result_file << number_of_resources<< ","<< vnode_id<< "," << num_rec_overlay_msg<< "," << overal_transcated_data_overlay<< "," <<  num_rec_discovery_msg << "," << overal_transcated_data_discovery << "," << num_rec_routing_msg << "," << num_rec_dis_routing_msg << ","<< endl; //num_rec_overlay3_msg<< "," << overal_transcated_data_overlay3<< endl ;
        txt_result_file.close();
    }
    else EV << "Unable to open file";
