...
std::vector<unsigned int> get_sub_query_type_ids_v2( std::string ls, std::string as, std::string ss){
    std::vector<unsigned int> type_ids;
    //PC-- Processor Type CPU=1, GPU=2, FPGA=3, N/A=4
    //INT --//Interconnection Network Bus=1, Ring=2, NOC=3, Crossbar=4, PointToPoint=5, HierarchicalNOC=6, Hypercube=7, Mesh=8, QsNet(Quadrics)=9 ,
    // Infiniband=10, Myrinet=11, SCI(Scalable Coherent Interface)=12, GE( Gigabit Ethernet)=13, NUMAlink=14, SP Switch=15, Proprietary=16, Cray Interconnect=17, Mixed=18, N/A=19
    ...
    // type_id=18  16<= MS < 32
    // type_id=19  32<= MS <64
    // type_id=20   64<= MS< 128
    // type_id=21   128 <= MS

    // type_id=22  NB< 100
    // type_id=23   100<= NB < 1000
    // type_id=24   1000<= NB < 10000
    // type_id=25   10000 <= NB < 20000
    // type_id=26   20000 <= NB
    ...
    if (as!=tempstr){
        ...
        EV<< "Values for as="<< as << "(" << PC<<INT<<ISA<<")"<< " = "<< PCi<< ", "<< INTi<< ", " << ISAi <<  endl;
    ...
    if (ss!=tempstr){
        ...
        unsigned int MSi, NBi ;


        std::istringstream iss8, iss9;
        ...
        if (MSi>0){
            if (MSi<16) type_ids.push_back(17);
            else if ((MSi>=16)&&(MSi<32)) type_ids.push_back(18);
            else if ((MSi>=32)&&(MSi<64)) type_ids.push_back(19);
            else if ((MSi>=64)&&(MSi<128)) type_ids.push_back(20);
            else type_ids.push_back(21);
        }
        ...
        if (NBi>0){
            if (NBi<100) type_ids.push_back(22);
            else if ((NBi>=100)&&(NBi<1000)) type_ids.push_back(23);
            else if ((NBi>=1000)&&(NBi<10000)) type_ids.push_back(24);
            else if ((NBi>=10000)&&(NBi<20000)) type_ids.push_back(25);
            else type_ids.push_back(26);
        }


        EV<< "Values for ss="<< ss << "(" << MS<<NB<<")"<< " = "<< MSi<< ", "<< NBi<<  endl;
    ...
    EV<< "Values for ls="<< ls << "(" << CCR<<L1S<<L1L<<L2S<< ")"<< " = "<< CCRi<< ", "<< L1Si<< ", " << L1Li << ", " << L2Si << endl;
